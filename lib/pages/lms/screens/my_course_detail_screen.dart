...
import '../lms_model/common_functions.dart';
import '../lms_model/course_db_model.dart';
import '../lms_model/lesson.dart';
import '../lms_model/section_db_model.dart';
import '../lms_model/video_db_model.dart';
...
class _MyCourseDetailScreenState extends State<MyCourseDetailScreen>
  ...
  void initState() {
    ...
    FileDownloader()
        .registerCallbacks(
      taskStatusCallback: myTaskStatusCallback, // Provide your task status callback function
      // taskProgressCallback: myTaskProgressCallback, // Optionally provide task progress callback function
    ...
    FileDownloader().updates.listen((update) async {
      switch (update.runtimeType) {
        case TaskStatusUpdate:
          TaskStatusUpdate statusUpdate = update as TaskStatusUpdate;
          if (statusUpdate.task == backgroundDownloadTask) {
            setState(() {
              downloadTaskStatus = statusUpdate.status;
            });
          }
        // case TaskStatusUpdate :
        ...
        case TaskProgressUpdate:
          TaskProgressUpdate progressUpdate = update as TaskProgressUpdate;
          progressUpdateStream.add(progressUpdate);
          break;
        // case TaskProgressUpdate :
        //   progressUpdateStream.add(update as TaskProgressUpdate);
        //   break;
      }
    });
  }
  ...
  void myTaskStatusCallback(TaskStatusUpdate statusUpdate) {
    Task task = statusUpdate.task;
    TaskStatus status = statusUpdate.status;
    debugPrint('Task Status $status for taskId $task');
  }

  void myTaskProgressCallback(TaskProgressUpdate progressUpdate) {
    Task task = progressUpdate.task;
    DownloadTask progress = progressUpdate.progress as DownloadTask;
    debugPrint('Download Video Progress Status $progress for taskId $task');
  ...
  Future<void> processButtonPress(lesson, myCourseId, coTitle, coThumbnail, secTitle, secId) async {
    ...
    if (mounted) {
      setState(() {
        path = "/data/user/0/com.dmrefer.mobile/files/system";
        fileName = lesson.title.toString();
        lessonId = lesson.id;
        courseId = myCourseId;
        sectionId = secId;
  ...
  void lessonAction(Lesson lesson) async {
    ...
    if (lesson.lessonType == 'video') {
      if (lesson.videoTypeWeb == 'html5' ||
        ...
        print("video url ${lesson.videoUrlWeb}");
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _isLoading
          ? Center(
        child: CircularProgressIndicator(color: kOrangeColor.withOpacity(0.7)),
      )
